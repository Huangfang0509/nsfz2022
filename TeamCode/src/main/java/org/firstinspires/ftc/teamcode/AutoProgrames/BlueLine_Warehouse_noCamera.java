...
//@Disabled
public class BlueLine_Warehouse_noCamera extends LinearOpMode {
    ...
    Pose2d p_catch = new Pose2d(37.5,65.2,Math.toRadians(0));//过程中值会变化
...
//        Trajectory moveToIniBack2 = driveBase.trajectoryBuilder(forwardForIntakeTwo.end(),true).lineToSplineHeading(iniBack).build();
        ...
        Trajectory sendToP_catch = driveBase.trajectoryBuilder(send,true)
                .lineToSplineHeading(transPose)
                .lineToSplineHeading(p_catch)
                .build();
//----------------------------------Trajectory build----------------------------------------------------------------------------------
    ...
    //step1,投放预载粒子
        ...
        //---------------------------第一次回取------------------------------------------------------
           ...
           driveBase.followTrajectoryAsync(forwardForIntakeOne);
            intakeOn();
            while (opModeIsActive() && driveBase.isBusy()){
                armDownPidUpdate(LEVEL_INI_POS);
            ...
            //回到缺口外切换点
            driveBase.followTrajectoryAsync(moveToPreSend1);
            while (opModeIsActive()&&driveBase.isBusy()){
